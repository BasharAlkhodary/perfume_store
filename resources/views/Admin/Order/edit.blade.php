...
<form class="mb-3 p-5" action="{{route('order.store')}}" method="post" enctype="multipart/form-data">
    ...
    <div class="mb-3">
        ...
        <input type="text" class="form-control" id="name" name="name" placeholder="Name Of order" value="{{ old('name', $order->name ?? '') }}">
    </div>
    <div class="mb-3">
        <label for="formFileSm" class="form-label">Total Price</label>
        <input class="form-control form-control-sm" id="total_price" name="total_price" type="number" value="{{ old('total_price', $order->total_price ?? '') }}">
    </div>
    ...
    <div class="mb-3">
        ...
        <textarea name="notes" class="form-control" id="notes" cols="20" rows="5" placeholder="Notes Of order">{{ old('notes', $order->notes ?? '') }}</textarea>
    </div>
    <br>
